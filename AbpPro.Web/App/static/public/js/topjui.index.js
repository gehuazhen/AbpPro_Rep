    ...
    if (menuId == "") {
        ...
        $.get(
            ...
            function (data) {
                ...
                $.each(data.MainMenu.Items, function (i, e) {// 循环创建手风琴的项
                    //var pid = e.pid;
                    var isSelected = i == 0 ? true : false;
                    $('#RightAccordion').iAccordion('add', {
                        fit: false,
                        title: e.DisplayName,//e.text,
                        content: "<ul id='tree" + e.Name + "' ></ul>",// e.id 
                        border: false,
                        selected: isSelected,
                        iconCls: e.iconCls
                    });
                    ...
                    $.each(e.Items, function (key, value) {
                        e.Items[key].id = e.Items[key].Name;
                        e.Items[key].pid = e.Name;
                        e.Items[key].text = e.Items[key].DisplayName;
                        e.Items[key].url = e.Items[key].Url; 
                        e.Items[key].iconCls = e.Items[key].Icon;
                        //e.Items[key].url = e.Items[key].DisplayName;
                    })

                    //$.get("/App/json/menu/menu_" + e.id + ".json", function (data) {// 循环创建树的项
                    $("#tree" + e.Name).tree({
                            data: e.Items, //  data,
                            lines: false,
                            animate: true,
                            onBeforeExpand: function (node, param) {
                                $("#tree" + e.Name).tree('options').url = "/App/json/menu/menu_" + node.Name + ".json";
                            },
                    ...
                    //        animate: true,
                    //        onBeforeExpand: function (node, param) {
                    //            $("#tree" + e.id).tree('options').url = "/App/json/menu/menu_" + node.id + ".json";
                    //        },
                    //        onClick: function (node) {
                    //            if (node.url) {
                    //                /*if(typeof node.attributes != "object") {

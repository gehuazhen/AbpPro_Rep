...
namespace AbpPro.Tests
{
    public abstract class AbpProTestBase : AbpIntegratedTestBase<AbpProTestModule>
    {
        ...
        protected void UsingDbContext(Action<AbpProDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AbpProDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AbpProDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AbpProDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AbpProDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpProDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AbpProDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpProDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AbpProDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AbpProDbContext, Task<T>> func)
        {
            T result;
